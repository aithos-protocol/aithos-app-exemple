...
export function Assets() {
  const { actor, capabilities } = useActor();
  if (!actor || !capabilities.canAssets) {
    return (
      <section>
        <h2>Assets</h2>
        <p className="warn"><strong>Owner only.</strong> Asset ownership uses your sphere keys; delegate asset access is a v0.2 target. (This demo still uses an ephemeral did:key — the SDK does not yet expose per-zone sphere seeds for a did:aithos owner.)</p>
      </section>
    );
  }
  return <AssetsDemo />;
