...
const PROMPT_TEMPLATE = (w: number, h: number) =>
  [
    ...
    "2. Locate y_pecTop = the TOP EDGE of the pectoral plates — the line",
    "   where the chest surface starts BULGING OUTWARD from the body. This",
    "   is NOT the collarbone, NOT the collar/neckline, and NOT y_neckBottom.",
    "   It is the visible start of the rounded muscle plate itself, which",
    "   on a stylised mascot typically sits a clear gap BELOW the collarbone",
    "   (think: the seam where a fitted t-shirt would crease over the chest).",
    "   If you can't see a clear bulge, place y_pecTop at roughly 1.2× the",
    "   distance from the top of the head to the chin, measured down from",
    "   y_neckBottom.",
    ...
    "4. Logo y = y_pecTop + 0.60 × (y_pecBottom − y_pecTop).",
    "   The 0.60 weighting places the emblem at HEART LEVEL — slightly",
    "   below the geometric midpoint of the pec plates, where Iron Man's",
    "   arc-reactor and Captain America's star actually sit. The geometric",
    "   midpoint reads as 'too high' on real bodies because the pec plate",
    "   is fuller at the top.",
    "   SANITY CHECK A: logo y MUST be clearly BELOW y_neckBottom — the gap",
    "   (logo_y − y_neckBottom) must be at LEAST 25% of (y_pecBottom −",
    "   y_neckBottom). If your logo y is sitting at the collar/neckline,",
    "   you've placed y_pecTop too high — recompute with y_pecTop lower.",
    "   SANITY CHECK B: logo y MUST be ABOVE y_pecBottom. If your computed",
    "   logo y is at or below y_pecBottom, or anywhere in the lower half of",
    "   the torso, you've picked the wrong landmarks — recompute. The logo",
    "   belongs on the muscle, NOT below it and NOT at the neckline.",
    ...
    '  "confidence": <0.0..1.0>,',
    '  "reasoning": "<sentence reporting y_neckBottom, y_pecTop, y_pecBottom, final logo y, the gap (logo_y − y_neckBottom) confirming Sanity Check A, that logo y is ABOVE y_pecBottom (Sanity Check B), and the chest colour you sampled>"',
    "}",
    "",
