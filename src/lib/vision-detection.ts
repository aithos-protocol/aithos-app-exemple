...
const PROMPT_TEMPLATE = (w: number, h: number) =>
  [
    ...
    "",
    "The logo sits on the LOWER-CHEST — at the same height where:",
    "  - the breast pocket of a shirt would be sewn",
    '  - the "arc-reactor" sits on Iron Man',
    "  - a heart-emblem would land on a superhero costume",
    "",
    "This is CLEARLY BELOW the geometric center of the chest panel.",
    "NEVER place it near the collarbone, the upper chest, or the neck.",
    "",
    ...
    "2. Locate y_pecBottom = the BOTTOM EDGE of the visible pectoral muscles —",
    "   the horizontal line where the rounded upper-chest curves transition",
    "   into the lower abdomen / waist. This is roughly nipple-level on a human.",
    "3. Locate y_plexus = the SOLAR PLEXUS — just below where the pectorals end,",
    "   where the rib cage starts to open into the abdomen.",
    "4. Logo y = (y_pecBottom + y_plexus) / 2.",
    "   This DELIBERATELY lands in the LOWER-CHEST region. If your computed",
    "   logo y is in the upper or middle half of the visible torso, you've",
    "   chosen the wrong anatomical landmarks — recompute.",
    ...
    "7. Logo diameter = round(chest_w × 0.70). This fills most of the chest",
    "   with a ~15% visual margin built-in around the disc. The logo should",
    "   read clearly at a glance — too small and it disappears, too large",
    "   and it overflows the chest panel.",
